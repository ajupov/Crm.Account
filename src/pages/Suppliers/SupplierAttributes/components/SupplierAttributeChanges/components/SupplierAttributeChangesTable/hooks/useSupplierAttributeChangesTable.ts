...
interface UseSupplierAttributeChangesTableReturn {
    page: number
    headers: TableHeaderCellProps[]
    map: (attributes: SupplierAttributeChange[]) => TableBodyRowProps[]
    onClickDownloadAsCsv: () => void
    onClickChangePage: (page: number) => void
}

// TODO: Move to l10n
const useSupplierAttributeChangesTable = (): UseSupplierAttributeChangesTableReturn => {
    ...
    const onClickDownloadAsCsv = useCallback(async () => {
        ...
        const fileName = getFileNameWithDateTime('История изменений атрибутов поставщика')
    ...
    const getChangeName = useCallback((change: SupplierAttributeChange) => {
    ...
    const getChangeValue = useCallback((change: SupplierAttributeChange) => {
        const oldValue = change.oldValueJson ? (JSON.parse(change.oldValueJson) as SupplierAttribute) : void 0
        const newValue = change.newValueJson ? (JSON.parse(change.newValueJson) as SupplierAttribute) : void 0
...
export default useSupplierAttributeChangesTable
