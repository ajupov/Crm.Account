...
interface UseTaskStatusChangesTableReturn {
    page: number
    headers: TableHeaderCellProps[]
    map: (statuses: TaskStatusChange[]) => TableBodyRowProps[]
    onClickDownloadAsCsv: () => void
    onClickChangePage: (page: number) => void
}

// TODO: Move to l10n
const useTaskStatusChangesTable = (): UseTaskStatusChangesTableReturn => {
    ...
    const getChangeName = useCallback((change: TaskStatusChange) => {
    ...
    const getChangeValue = useCallback((change: TaskStatusChange) => {
        const oldValue = change.oldValueJson ? (JSON.parse(change.oldValueJson) as TaskStatus) : void 0
        const newValue = change.newValueJson ? (JSON.parse(change.newValueJson) as TaskStatus) : void 0
...
export default useTaskStatusChangesTable
